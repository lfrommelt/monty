...
\addbibresource{01_Introduction/references.bib}
...
\begin{frame}{Structure}
    ...
    \note{
        \begin{itemize}
            ...
            \item \textbf{Feedback:} You can get individual feedback on your homework submission (see a few slides below)
        \end{itemize}
    }
...
\begin{frame}{Homework and Grading}
    ...
    \begin{block}{\textbf{Feedback Sessions}}
    \begin{enumerate}
        \item Detailed feedback will be given in sessions on Tuesdays
        \item Each week time slots will be assigned individually
        \item Request session by entering the timeslot group on StudIP
        \item We will ensure fair distribution of feedback sessions
    \end{enumerate}
    \end{block}

    \note{
        \begin{itemize}
            \item Timeslots for feedback sessions are on Tuesday between 9:00 to 12:00
            \item If you want feedback for your homework submission, you can apply for a timeslot by assigning yourself to the corresponding group on StudIP (\textbf{ONLY ONE PERSON PER GROUP, PLEASE})
            \item You need to re-apply each week if you want feedback
            \item We probably cannot accommodate all of you, so we will give preference to groups who have had less feedback sessions so far
        \end{itemize}
    }
...
\begin{frame}[fragile]{\texttt{print("hello world!")}}
    ...
    \note{
        ...
        \begin{itemize}
            \item \textbf{Windows:} Press \keys{\OS + R} and type \texttt{cmd}. Hit \keys{\return}.
